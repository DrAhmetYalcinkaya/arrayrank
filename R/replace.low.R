...
#' @param fix A numeric value used when `strategy` is `"fixed"`. Default is `offset`.
#' @param verbose A logical value indicating whether to print the number of replacements made. Default is TRUE.
#'
...
#' }
replace.low <- function(data, offset = 10, strategy = "computed", fix = offset, verbose = TRUE){
  ...
  if (verbose) {
    print(paste("Replaced", replaced_n, "values that are lower than the selected offset value of", offset, "with", strategy, "strategy"))
  }
